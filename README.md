# 🛒 Bodegón Bayona 59 - Tienda Online

Sitio web oficial de Bodegón Bayona 59, tienda de combos, materiales de construcción y productos del mercado en La Habana, Cuba.

## 🌐 Sitio Web
🔗 https://jccarlos010277-cmd.github.io/bayona59/

## 🚀 Características
- ✅ Slider interactivo con 3 promociones
- ✅ Combos de alimentos con descuentos
- ✅ Materiales de construcción (cavillas de acero)
- ✅ Diseño moderno y 100% responsive
- ✅ Carrito de compras funcional
- ✅ Formulario de contacto
- ✅ Optimizado para móviles

## 🛍️ Productos Destacados
- **Super Combo Familiar** - 25% de descuento
- **Cavillas de acero** - Diferentes medidas
- **Productos del mercado** - Calidad garantizada

## 🛠️ Tecnologías
- **HTML5** - Estructura semántica
- **CSS3** - Diseño moderno con variables CSS
- **JavaScript** - Interactividad
- **Swiper.js** - Slider profesional
- **Font Awesome** - Iconografía

## 📞 Contacto
- 📍 **Ubicación:** Calle Bayona #59, La Habana, Cuba
- 📱 **WhatsApp:** +53 987 654 32
- 🕒 **Horario:** Lunes a Viernes 9am-6pm


## 📁 Estructura del Proyecto
